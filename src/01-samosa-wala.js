...
export function createSamosaCart(ownerName, location) {
  ...
  const cart = {
    ...
    sales: [],
    sellItem(itemName, quantity) {
      if (!Object.hasOwn(this.menu, itemName) || quantity <= 0) {
        return -1;
      }
      const total = this.menu[itemName] * quantity
      this.sales.push({ item: itemName, quantity: quantity, total: total })
      return total
    },
    getDailySales() {
      ...
      return this.sales.reduce((sum, obj) => sum + (obj.total || 0), 0);
    },
    getPopularItem() {
      if (this.sales.length === 0) {
        return null
      }
      const totals = this.sales.reduce((acc, obj) => {
        acc[obj.item] = (acc[obj.item] || 0) + obj.quantity;
        return acc
      }, {});

      const maxItem = Object.keys(totals).reduce((a, b) =>
